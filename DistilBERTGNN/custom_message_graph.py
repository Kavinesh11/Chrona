...
# DistilBERTGNN: unified node filtering dispatcher (sentiment or centrality)
from node_filter import apply_node_filter
from args import args_define as _args_define
_args = _args_define.args
...
def to_dgl_graph_v3(G, save_path=None):
    ...
    start = time()
    A = nx.to_numpy_array(G)  # Returns the graph adjacency matrix as a NumPy array.
    mins = (time() - start) / 60
    print('\tDone. Time elapsed: ', mins, ' mins\n')
    message += '\tDone. Time elapsed: '
    message += str(mins)
...
def construct_incremental_dataset_0922(df, save_path, features, test=True,
    ...
    # Fall back to CLI args when caller does not override
    filter_method         = filter_method         if filter_method         is not None else _args.filter_method
    sentiment_threshold   = sentiment_threshold   if sentiment_threshold   is not None else _args.sentiment_threshold
    centrality_percentile = centrality_percentile if centrality_percentile is not None else _args.centrality_percentile
    ...
    # --- DistilBERTGNN: Node Filtering (Section 4.4, Algorithm 1 line 6) ---
    # For centrality filtering we need the graph built first to compute centralities.
    _G_for_centrality = construct_graph_from_df(ini_df) if filter_method == 'centrality' else None
    ini_df, filter_mask, _ = apply_node_filter(
        ini_df,
    ...
    for i in range(7, len(distinct_dates) - 1):
        ...
        # --- DistilBERTGNN: Node Filtering (Section 4.4, Algorithm 1 line 6) ---
        _G_for_centrality = construct_graph_from_df(incr_df) if filter_method == 'centrality' else None
        incr_df, filter_mask, _ = apply_node_filter(
            incr_df,
            method=filter_method,
            G=_G_for_centrality,
            sentiment_threshold=sentiment_threshold,
...
# load features — prefer DistilBERT (770-dim), fall back to spaCy (302-dim)
_distilbert_features_path = '../datasets/Twitter/features_distilbert_0709_multiclasses_filtered.npy'
_spacy_features_path = '../datasets/Twitter/features_69612_0709_spacy_lg_zero_multiclasses_filtered.npy'
if os.path.exists(_distilbert_features_path):
    f = np.load(_distilbert_features_path)
    print(f"Loaded DistilBERT features: {f.shape}")
else:
    print("[WARNING] DistilBERT features not found, falling back to spaCy features.")
    print("[WARNING] Run generate_initial_features.py first to generate DistilBERT features.")
    f = np.load(_spacy_features_path)
    print(f"Loaded spaCy features: {f.shape}")
