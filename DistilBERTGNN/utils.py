...
def extract_embeddings(g, model, num_all_samples, labels):
    # Ensure g is on the same device as model
    device = next(model.parameters()).device
    # Create sampler
    sampler = dgl.dataloading.MultiLayerNeighborSampler([1000, 1000]) # 2 hops, 1000 neighbors
    
    # Create DataLoader
    # We use batch_size=num_all_samples to mimic the original behavior of one single large batch
    ...
    with torch.no_grad():
        ...
        for input_nodes, output_nodes, blocks in dataloader:
            # Load features
            # blocks[0].srcdata['features'] might not be populated automatically unless we ask for it
            # But we can get it from g using input_nodes
            input_features = g.ndata['features'][input_nodes.cpu()].to(device)
            
            blocks = [b.to(device) for b in blocks]
            
            if args.use_dgi:
                extract_features, _ = model(blocks, input_features)  # representations of all nodes
            else:
                extract_features = model(blocks, input_features)  # representations of all nodes
            
            extract_nids = output_nodes.to(device=extract_features.device, dtype=torch.long)  # node ids
            extract_labels = labels[extract_nids.cpu()]  # labels of all nodes
            
            # We expect only one batch since batch_size = num_all_samples
            break
        ...
        # print("A", A)
        # assert (A == extract_nids).all() # Sorting might differ, so we skip this assertion or strict check
...
def run_kmeans(extract_features, extract_labels, indices, isoPath=None):
    ...
    # Return number of test tweets, number of classes, NMI, AMI, ARI
